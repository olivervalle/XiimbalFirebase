...
export const seedFirebase = async (): Promise<void> => {
  try {
    ...
    if (businessIds.length > 0) {
      ...
      // Assign all reviews to the first business
      for (const review of sampleReviews) {
        const reviewWithBusinessId = { ...review, businessId: businessIds[0] };
        const reviewDoc = await addDoc(reviewsRef, reviewWithBusinessId);
        console.log(
          `Added review for business: ${businessIds[0]} with ID: ${reviewDoc.id}`,
        );
      }

      // Add one review to the second business if it exists
      if (businessIds.length > 1) {
        const secondBusinessReview = {
          userId: "user4",
          userName: "David Rodriguez",
          rating: 5,
          comment:
            "The technicians were extremely knowledgeable and fixed my computer issues quickly. Fair pricing and excellent customer service.",
          date: new Date(),
          businessId: businessIds[1],
        };
        const reviewDoc = await addDoc(reviewsRef, secondBusinessReview);
        console.log(
          `Added review for business: ${businessIds[1]} with ID: ${reviewDoc.id}`,
        );
      }
    }
...
// Function to get all businesses from Firebase
...
export const getCategoriesFromFirebase = async (): Promise<string[]> => {
  try {
    const businessesRef = collection(db, "businesses");
    const snapshot = await getDocs(businessesRef);

    if (snapshot.empty) {
      return sampleBusinesses.map((b) => b.category);
    }
...
// Function to get locations from Firebase
export const getLocationsFromFirebase = async (): Promise<string[]> => {
  try {
    const businessesRef = collection(db, "businesses");
    const snapshot = await getDocs(businessesRef);

    if (snapshot.empty) {
      return sampleBusinesses.map((b) => b.city);
    }
